...
contract('7. Payout Covee Fee', (accounts) => {
    ...
    it('6.2.1 Team member payouts have been allocated to the fractions of Covee Token Payout', async () => {
        ...
        sumMilestonePayouts = payClient + payMember1 + payMember2;
    });

    it('7.1.2a Stage is correct', async () => {
        await expectThrow(
            covee.payoutCoveeFee(
                projectId
            )
        );
    });
    ...
    it('7.1.1 _projectId must exist', async () => {
    ...
    it('7.2.3 Covee Payout can’t be done twice', async () => {
        await expectThrow(
            covee.payoutCoveeFee(
                projectId
            )
    ...
    it('Covee transaction fee to the beneficiary covee address approved', async () => {
        const payoutCovee = await coveeToken.allowance(covee.address, beneficiary);
        const payCovee = payoutCovee.toNumber() / 1e18;
        console.log('payout covee fee (aka covefee): ', payCovee);
    });

    it('7.2.2 Staked Tokens can be payed out afterwards', async () => {
    ...
    it('Allocation of staked tokens payout should be done for team member 2', async () => {
        const tx0 = await covee.payoutStakedTokens(
            projectId,
            2,
            {from: teamMember2}
